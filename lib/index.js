...
function emailPreCompiler(options, callback) {
    ...
    var srcFolder = options.src,
        ...
        writeTextVersion = options.writeTextVersion || true;
    ...
    function writeCompiledEmails() {
        ...
        try{
            _(preCompiledEmails).each(function(templateContent, templateName) {
                ...
                //html
                log("writing '" + templateName + "' -> " + languageDir + "/" + templateName + ".html");
                fs.writeFileSync(path.join(languageDir, templateName + ".html"), templateContent.html, "utf-8");

                //text
                if(templateContent.text !== undefined && writeTextVersion) {
                    log("writing '" + templateName + "' -> " + languageDir + "/" + templateName + ".txt");
                    fs.writeFileSync(path.join(languageDir, templateName + ".txt"), templateContent.text, "utf-8");
                }
            });
