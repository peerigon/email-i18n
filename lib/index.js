...
function getGlobalContent (contentDir, language) {
    ...
    function loadContentSnippets (snippetDir, prefix) {
        ...
        _(snippets).each(function(snippet) {
            ...
            if(snippet.indexOf(".html") !== -1) {
                loadedSnippets[prefix + snippet.substr(0, snippet.length - 5)] = fs.readFileSync(snippetDir + "/" + snippet);
            }
            else if(snippet === language){
                _(loadedSnippets).extend(loadContentSnippets(contentDir + "/" + snippet, "_"));
            }
        });
    ...
    //console.log("Gobal Content", _(globalContent).keys());
    return globalContent;
}
...
function loadEmailContent(contentDir, emailTitle, language) {
    ...
    //console.log("loading lang-specific stuff at ", emailTitle, langSpecificContentPath);
    ...
    //console.log("local snippets", contentMap);


    _(contentMap).extend(getGlobalContent(contentDir, language));

    //console.log("Email Content", emailTitle, _(contentMap).keys());
